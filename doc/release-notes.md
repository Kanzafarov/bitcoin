Bitcoin Knots version *0.17.1.knots20181228* is now available from:

  <https://bitcoinknots.org/files/0.17.x/0.17.1.knots20181228/>
...
| `listtransactions`     | The `account` named parameter has been renamed to `label`. If provided, only incoming transactions will be returned unless running with the `-deprecatedrpc=accounts` argument (in which case functionality is unchanged). |
...
- The RPC `scantxoutset` can now be used to scan the UTXO set for entries
  ...
  wallet can be disabled at compile or run time. This call is no longer
  compatible with the same method in older versions.

- The `createrawtransaction` RPC will now accept an array or dictionary (kept for compatibility) for the `outputs` parameter. This means the order of transaction outputs can be specified by the client.
- The `fundrawtransaction` RPC will reject the previously deprecated `reserveChangeKey` option.
- `sendmany` now shuffles outputs to improve privacy, so any previously expected behavior with regards to output ordering can no longer be relied upon.
- The `getsignaturehash` RPC is no longer supported and has been removed.
...
- `fundrawtransaction` now accepts a `min_conf` parameter to limit funding to sufficiently confirmed coins.
...
- The `getpeerinfo` RPC method now reports the peer's current `minfeefilter` value.
- The `getaddressinfo` RPC method now includes an `ischange` key to indicate if the wallet considers it to be change.
...
- The `size` parameter on mempool-related calls has been deprecated and renamed
  to `vsize`.

- The `dumpwallet` RPC method now returns an `hdseedid` value instead of the incorrectly-named `hdmasterkeyid` value (which has already been removed in this release).
...
- The `address_filter` parameter for `listreceivedbyaddress` used to be called `only_address`. This old name is no longer supported.
- The `key_origin` field returned by the `listreceivedby*` RPC methods has been deprecated and will only be returned with `-deprecatedrpc=key_origin`. It may be removed in a future release.
...
- n/a    `79ad014` *Update checkpoints and chain params, adding a new checkpoint at block 553,472 (luke-jr)
- #2241  `007c293` *dbwrapper: Limit max_open_files to a safe number when using system LevelDB (luke-jr)
...
- #14970 `7f0881a` *[net] add dnsseed.emzy.de to DNS seeds (Emzy)
...
- #14380 `5bbb4f5` *CreateTransaction: Assume minimum p2sh-p2wpkh spend size for unknown change (instagibbs)
- #14383 `6ffb8b9` *Clean systray icon menu for -disablewallet mode (hebasto)
- #15023 `cc8fe17` *Restore RPC Console to non-wallet tray icon menu (luke-jr)
- #5916  `3433e14` *Never return origin info for key metadata if we can't store it (luke-jr)
...
- n/a              *scantxoutset support for raw sweep transactions has been dropped, and the RPC interface has changed substantially
- n/a              *getsignaturehash RPC method has been dropped
- n/a              *listreceivedbyaddress's address_filter parameter no longer accepts the older only_address name
- #9332  `a090a80` *Let wallet importmulti RPC accept labels for standard scriptPubKeys (ryanofsky)
- #13008 `e234577` *Rename size to vsize in mempool related calls (Miguel Herranz)
- #13152 `74c2179` *Add getnodeaddresses RPC command (chris-belcher)
- #13987 `79f8efe` *Report minfeefilter value in getpeerinfo rpc (Anthony Towns)
- #14060 `c84c99e` *ZMQ: add options to configure outbound message high water mark, aka SNDHWM (mruddy)
- #14410 `dbdcef3` *rpcwallet: add 'ischange' field to 'getaddressinfo' response (whythat)
- #14641 `5f6e800` *rpc: Add min_conf option to fundrawtransaction and walletcreatefundedpsbt (João Barbosa)
- #14687 `ebd0cd8` *zmq: enable tcp keepalive (mruddy)
- #11803 `5092fca` *RPC/Wallet: dumpwallet: Replace hdmasterkeyid with hdseedid (luke-jr)
- n/a    `07deac7` *RPC/Wallet: Hacky fix for getbalance bugs (luke-jr)
- #5916  `32d920a` *RPC/Wallet: Deprecate key_origin in listreceivedby* (luke-jr)
- #5916  `acad9c9` *RPC/Wallet: Don't include key_origin when it would be a null string (no flags set) (luke-jr)
- #14968 `a75a3a8` *http: Fail initialization when any bind fails (laanwj)
- n/a    `d107faa` *Workaround issues in #14968 (luke-jr)
...
- #14133 `1626725` *Favor macOS show / hide action in dock menu (João Barbosa)
- #14137 `c1131db` *Add Windows taskbar progress (ken2812221)
- #14228 `e43e1d8` *Don't use systray icon on inappropriate systems (hebasto)
- #14517 `b304841` *Fix start with the `-min` option (hebasto)
- #14573 `b992e51` *Add Window menu (João Barbosa)
- n/a    `317c728` *Qt/Options: Replace prune setting with rwconf-based version (luke-jr)
...
- n/a              *libevent is now required to build, as the patch to make it optional has been dropped
- #13724 `c0029ee` *Support ARM and RISC-V symbol check (ken2812221)
- #14066 `bbbdda3` *gitian-linux: Build binaries for 64-bit POWER (luke-jr)
- #14564 `2923628` *Adjust configure so that only bip70 is disabled when protobuf is missing instead of the GUI (Lightsword)
...
- n/a    `5fc505b` *qa: interface_bitcoin_cli: Adapt for Knots (luke-jr)
- n/a    `4766ce2` *QA: Adapt feature_includeconf test for complex UAs (luke-jr)
- #14036 `9d6e1f2` *depends: allow CC/CXX to be overridden during configure (MarcoFalke)
- #14036 `2c3a286` *travis: Run unit tests --with-sanitizers=undefined (MarcoFalke)
- #14818 `27603be` *Bugfix: test/functional/rpc_psbt: Remove check for specific error message that depends on uncertain assumptions (luke-jr)
- #14819 `29aeed1` *Bugfix: test/functional/mempool_accept: Ensure oversize transaction is actually oversize (luke-jr)
...
- #13910 `e58985c` *Log progress while verifying blocks at level 4. (Daniel Kraft)
